...
import shutil
import os.path
from cityscape_preprocessing import make_json2txt

#
def gather_json(gtFine_dir,type):
    if type == 'train' or type =='test':
        dest_dir = 'CITYSCAPESdevkit/trainval_json/train'
    else:
        dest_dir = 'CITYSCAPESdevkit/trainval_json/validation'
...
def gather_png(leftImg_dir, data_path,type):
    if type == 'train' or type =='test':
        dest_dir = os.path.join(data_path,'images','train')
    else:
        dest_dir = os.path.join(data_path,'images','validation')
...
def make_filelist_txt(abs_path):

    # abs_path -> cityscapes 폴터
    traintxt_dir = os.path.join(abs_path,'images','train')
    valtxt_dir = os.path.join(abs_path,'images', 'validation')

    t_list = os.listdir(traintxt_dir)
    v_list = os.listdir(valtxt_dir)

    train_txt_path = os.path.join(abs_path,'cityscapes_train.txt')
    val_txt_path = os.path.join(abs_path,'cityscapes_validation.txt')

    with open(train_txt_path, 'w') as ft:
        for i in t_list:
            ft.write(os.path.join(traintxt_dir,i)+'\n')
    with open(val_txt_path, 'w') as fv:
        for i in v_list:
            fv.write(os.path.join(valtxt_dir,i)+'\n')

    return  train_txt_path, val_txt_path


if __name__ == '__main__':
    gtFine_dir = "CITYSCAPESdevkit/cityscapes/gtFine_trainvaltest/gtFine"
    leftImg_dir = "CITYSCAPESdevkit/cityscapes/leftImg8bit_trainvaltest/leftImg8bit"
    # set the save path
    data_path = 'cityscapes'  # yolov7 데이터 파일 만드는 디렉토리
    ...
    train_txt, val_txt = make_filelist_txt(abs_path)
